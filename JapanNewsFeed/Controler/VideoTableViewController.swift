...
class VideoTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        self.navigationController?.navigationBar.setBackgroundImage(navBackgroundImage,
                                                                    for: .default)
        defaultRowHeight = Int(screenWidth * 0.3)
        noPicDefaultHeight = Int(screenWidth * 0.3)
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        performSegue(withIdentifier: "ShowVideoSegue", sender: newsFeedArray[indexPath.row].link)
    ...
    func retrieveData(numberitems: Int){
        ...
        Alamofire.request(Const.GET_MESSAGE_BY_VIDEO, method: .post, parameters: parameters).responseJSON { response in
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if(newsFeed.has_image){
            let cell = Bundle.main.loadNibNamed("SmallPicTableViewCell", owner: self, options: nil)?.first as! SmallPicTableViewCell
            ...
            if let url = URL(string: newsFeed.image_url!) {
                cell.picImageView.contentMode = .scaleAspectFit
                downloadImage(url: url, imageView: cell.picImageView)
            }
            return cell
        }else{
