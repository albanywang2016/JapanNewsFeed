...
//  JapanNewsFeed
//
//  Created by Lei Wang on 12/29/17.
...
class NewsViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        defaultRowHeight = Int(Const.screenWidth * 0.9)
        noPicDefaultHeight = Int(Const.screenWidth * 0.35)
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        performSegue(withIdentifier: "ShowNewsSegue", sender: newsFeedArray[indexPath.row].link)
    ...
    func retrieveData(numberitems: Int){
        ...
        Alamofire.request(Const.GET_MESSAGE_BY_NEWS, method: .post, parameters: parameters).responseJSON { response in
            ...
            case .success(let data):
                ...
                for(index, dict) in jsonArray{
                    var title = dict["title"].stringValue

                    var pub_date = dict["pub_date"].stringValue as String
                    //let oneInstance = Const()
                    //var pub_date = oneInstance.transformDay(input_date: date_before_change)
